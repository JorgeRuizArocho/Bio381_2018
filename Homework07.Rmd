...
ANOPlot <- ggplot(data=HerbData,
                  aes(x=variable, y=value, fill=value)) + geom_boxplot()
print(ANOPlot)
```
...
myDF <- HerbivoryArea(mean1=13, mean2 = 10) # In this fuction we can change our parameters to see how small the diffeences between the groups can be, in our example a small change in the mean still results in a significat p-value, but if we use relative different means, the chances of obtaining significant p-values increases.
head(myDF)
...
myDF <- HerbivoryArea(n1=50, n2= 50) # In this fuction we can change our parameters to see the minimun sample size needed to observe a statistically significant effect. Again, a large set of observations (n) give us a better chance of obtaining significant p-values.
head(myDF)
